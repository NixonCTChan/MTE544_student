...
def generate_sample_points(start, goal, rr, obstacles_list, obstacle_kd_tree, rng):
    ...
    # Include start and goal points explicitly
    ...
    while len(sample_x) < N_SAMPLE:
        ...
        # Check if sample point is valid (not in collision)
...
def is_collision(sx, sy, gx, gy, rr, obstacle_kd_tree, max_edge_len):
    ...
    for i in range(steps + 1):
        x = sx + i * dx / steps
        y = sy + i * dy / steps
        d, _ = obstacle_kd_tree.query([x, y])
        if d <= rr:
            return True
...
def generate_road_map(sample_points, rr, obstacle_kd_tree, max_edge_len, m_utilities=None):
    ...
    for i, (sx, sy) in enumerate(zip(sample_x, sample_y)):
        neighbors = kd_tree.query([sx, sy], k=N_KNN + 1)[1][1:]  # Exclude self
        connections = []
